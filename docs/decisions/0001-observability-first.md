...
- Signalglass cannot observe live traffic until a later capture/proxy milestone.
- Users must export or save run data to analyze it in early versions.
- Token counts remain estimates until a real tokenizer or provider metadata is integrated.

### Accepted trade-offs

- Early versions require manual input files. Live capture will come later.
