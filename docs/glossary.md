...
## Evidence primitive
A canonical TypeScript representation of a Spec 013 evidence record (trace envelope, span, event, request/response envelope, context artifact, context contribution, completeness record, or their vocabulary types), as defined by [Spec 014 — Evidence primitives](../specs/014-evidence-primitives.md) (Accepted). The Spec 014 foundation slice — the dependency-free [`@signalglass/evidence`](../packages/evidence/README.md) package of types, validators, and JSON-safe serialization — is implemented; the compatibility-projection and migration slices remain pending. Evidence primitives are added beside the existing v0.x runtime models, are provider-neutral, and carry runtime validation and JSON-safe serialization; they are not measurements or interpretations.
