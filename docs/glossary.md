...
## Content hash
A SHA-256 digest (`sha256:` + 64 lowercase hex) whose input is the retained payload representation, hashed as bytes: `byte_faithful` bytes are hashed directly (never treated as UTF-8 text); `structurally_faithful` JSON is canonicalized with RFC 8785 (JCS) and hashed as UTF-8; other structured formats use their declared `contentCanonicalizer` (name + version) and its output encoding; with no deterministic canonicalizer, no hash is emitted. It hashes only retained payload content, never metadata, and never implies possession of discarded original content.
