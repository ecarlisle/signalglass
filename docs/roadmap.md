...
| Append-only evidence store | #21 | Aug 12–18 | Persist and retrieve canonical records without overwriting authoritative observations. Drafted as [Spec 015 — Append-only evidence store](../specs/015-append-only-evidence-store.md) (**Draft** — append-only save/retrieve beside the legacy `TraceStorage`, authoritative identity, exact-text conflict resolution, a mandatory non-bypassable storage-safety gate with a closed deterministic S1–S5 contract, the conservative `metadata-safe` reference persistence policy with field-level content classification aligned to the actual evidence vocabularies and raw-observation/projected-event shapes, runtime-validated policy decisions with closed storage-owned codes, stored-versus-in-memory parity at the serializer snapshot, clock-independent idempotency classification, read integrity verified before any `unsupported-version` result, atomic initialization with rollback, and a namespaced storage-format ledger); implementation is prohibited until the spec is reviewed and marked Accepted. |
