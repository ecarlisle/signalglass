...
- Spec 016 is **Accepted** in documentation-only PR #23; implementation is now permitted.
...
| Streaming ingress and trace assembly | #23 | Aug 17–21 | Assemble and persist one canonical `EvidenceRecord` per streaming OpenAI-compatible interaction (client request → upstream dispatch → response headers → ordered SSE chunks → usage → finish reason → `[DONE]` → errors/cancellation → lifecycle/completeness declarations), per [Spec 016](../specs/016-streaming-ingress-trace-assembly.md) (Accepted in documentation-only PR #23; S1 schema foundation implemented in PR #24; S2 `metadata-safe` v1.1.0 persistence policy implemented in PR #25; S3–S5 pending). |
...
- Transparent streaming capture (SSE passthrough with event extraction), so real agent-harness traffic (Pi, OpenCode) can be observed without being changed (see [Spec 016](../specs/016-streaming-ingress-trace-assembly.md), Accepted in documentation-only PR #23; implementation permitted).
