...
| Append-only evidence store | #21 | Aug 12–18 | Persist and retrieve canonical records without overwriting authoritative observations. Drafted as [Spec 015 — Append-only evidence store](../specs/015-append-only-evidence-store.md) (**Draft** — append-only save/retrieve beside the legacy `TraceStorage`, authoritative identity, structured conflicts, persistence-policy admission, storage manifest); implementation is prohibited until the spec is reviewed and marked Accepted. |
