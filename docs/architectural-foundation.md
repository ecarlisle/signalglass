...
Specifications, ADRs, and other documents that describe the current v0.x implementation (notably the `Trace`/`TraceEvent` and `AgentRun` models and optimization-oriented features) remain accurate records of what exists, but they are **no longer authoritative for the target architecture**. A later, separately accepted evidence-model specification will formally supersede them.
