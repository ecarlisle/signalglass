...
| [016](016-streaming-ingress-trace-assembly.md) | Streaming ingress and trace assembly | **Draft** — proposed; implementation prohibited until accepted. Defines how the OpenAI-compatible ingress observes a streaming interaction and assembles one canonical `EvidenceRecord` (client request → upstream dispatch → response headers → ordered SSE chunks → usage → finish reason → `[DONE]` → errors/cancellation → lifecycle/completeness declarations), with sixteen decision blocks: the two-lifecycle separation (client passthrough vs. evidence observation) with both lifecycle outcomes (observation terminal + transport end/delivery) persisted independently, the single-save assembly/persistence boundary with the honest crash limitation, deterministic `seq`/identity ordering (never timestamps or content-hash identity), byte/order-transparent backpressured passthrough with no silent frame mutation across four observation layers, the full SSE parsing/terminalization matrix, the closed evidence-status vocabulary with declared losses (no fabricated zeros/finish reasons/success), metadata-safe capture and persistence defaults (env-var-only API keys, no raw payloads by default), legacy coexistence with divergence detection, the deterministic error/cancellation state machine with observable storage failures, package boundaries (proposed `@signalglass/streaming`; network in `apps/ingress`, provider decoding in `@signalglass/providers`, persistence in `@signalglass/storage`, projections in `@signalglass/core`), and closed public contracts with explicit versioning. Drafted in documentation-only PR #23 and revised by documentation-only correction passes (revisions 2 and 3) — forecast only, not accepted |
