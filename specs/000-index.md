...
| [016](016-streaming-ingress-trace-assembly.md) | Streaming ingress and trace assembly | **Accepted — S1 implemented in PR #24 and S2 implemented in PR #25; S3–S5 pending.** S1 adds the schema-1.1 evidence contracts, version-aware validation, streaming lifecycle/loss derivation, bounded record validation, safe additive-field preservation, and genuine schema-1.0 compatibility. S2 adds explicit `metadata-safe` v1.1.0 persistence admission with per-leaf declarations, bounded captured paths, unchanged v1.0.0 behavior, and truthful schema-1.0 delegation. SSE parsing, provider decoding/assembly, and ingress wiring remain scoped to S3–S5. |
