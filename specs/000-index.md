...
| [016](016-streaming-ingress-trace-assembly.md) | Streaming ingress and trace assembly | **Accepted — S1–S4 implemented; S5 pending.** S1 (PR #24) adds the schema-1.1 evidence contracts and version-aware validation. S2 (PR #25) adds explicit `metadata-safe` v1.1.0 persistence admission while preserving v1.0.0 behavior. S3 adds the network-free incremental, byte-bounded SSE parser. S4 adds the OpenAI SSE L3 decoder and the provider-neutral deterministic evidence assembler, including leaf-owned detect-then-retain normalization, closed terminal classification, Spec 014 atomic collapse, and evidence-budget/finalization contracts. Live streaming route wiring, byte passthrough/backpressure, saving, and projection integration remain S5; this status does not claim live streaming ingress. |
