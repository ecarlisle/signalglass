...
| [016](016-streaming-ingress-trace-assembly.md) | Streaming ingress and trace assembly | **Accepted — S1–S3 implemented; S4–S5 pending.** S1 (PR #24) adds the schema-1.1 evidence contracts and version-aware validation. S2 (PR #25) adds explicit `metadata-safe` v1.1.0 persistence admission while preserving v1.0.0 behavior. S3 adds the network-free `@signalglass/streaming` package and its incremental, byte-bounded SSE parser with strict terminal, metadata-loss, malformed-stream, overflow, and post-terminal facts. Provider decoding/assembly and ingress wiring remain scoped to S4–S5; this status does not claim live streaming ingress. |
