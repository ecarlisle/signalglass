...
| [015](015-append-only-evidence-store.md) | Append-only evidence store | **Draft** — canonical `EvidenceRecord` persistence beside the legacy `TraceStorage` (append-only save/retrieve, authoritative identity, structured conflicts, persistence-policy admission boundary, storage manifest, storage-format versioning). Not Accepted; implementation is prohibited until reviewed and marked Accepted |
