...
| [015](015-append-only-evidence-store.md) | Append-only evidence store | **Draft** — canonical `EvidenceRecord` persistence beside the legacy `TraceStorage` (append-only save/retrieve, authoritative identity, exact-text conflict resolution, mandatory non-bypassable storage-safety gate with a closed deterministic S1–S5 contract, conservative `metadata-safe` reference persistence policy with field-level content classification aligned to the actual evidence vocabularies and raw-observation/projected-event shapes, runtime-validated policy decisions with closed storage-owned codes, stored-versus-in-memory parity at the serializer snapshot, clock-independent idempotency classification, read integrity verified before any `unsupported-version` result, atomic initialization with rollback, namespaced storage-format ledger). Not Accepted; implementation is prohibited until reviewed and marked Accepted |
