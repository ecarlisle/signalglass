...
| [015](015-append-only-evidence-store.md) | Append-only evidence store | **Draft** — canonical `EvidenceRecord` persistence beside the legacy `TraceStorage` (append-only save/retrieve, authoritative identity, exact-text conflict resolution, mandatory non-bypassable storage-safety gate, conservative `metadata-safe` reference persistence policy with a normative field/category matrix, stored-versus-in-memory parity at the serializer snapshot, storage-safe leak-free policy and validation results, single fixed policy-error outcome, read integrity verified before any `unsupported-version` result, atomic initialization with rollback, namespaced storage-format ledger). Not Accepted; implementation is prohibited until reviewed and marked Accepted |
